...
export class LCPObserver extends BaseObserver {
  ...
  // LCP评分阈值（毫秒）
  private static readonly LCP_GOOD_THRESHOLD = 2500;
  private static readonly LCP_NEEDS_IMPROVEMENT_THRESHOLD = 4000;
   ...
   * 为LCP指标分配评级
   */
  private assignLCPRating(value: number): 'good' | 'needs-improvement' | 'poor' {
        ...
        entries.forEach(entry => {
          ...
          if (entry.startTime < this.firstHiddenTime) {
            ...
            const lcp: MetricData = {
              ...
              context: {
                elementId: (entry as any).element ? (entry as any).element.id || null : null,
                elementTagName: (entry as any).element ? (entry as any).element.tagName || null : null,
                elementType: (entry as any).element ? (entry as any).element.type || null : null,
   ...
   * 设置停止监听的各种条件
   */
  private setupStopListening(): void {
    // 用于确保只报告一次的函数
    const stopListening = () => {
      if (this.metricReported || !this.lcpObserver) return;
    ...
    // 监听用户交互事件（键盘和点击）
    const addInteractionListener = (type: string) => {
      if (typeof document === 'undefined') return;
